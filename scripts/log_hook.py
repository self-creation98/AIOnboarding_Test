...
def main():
    ...
    parser = argparse.ArgumentParser(description="AI hook logger")
    parser.add_argument("--json", dest="json_str", help="JSON payload (alternative to stdin)")
    parser.add_argument("--prompt", help="Prompt summary text")
    parser.add_argument("--response", help="Response summary text")
    parser.add_argument("--event", default="Notification", help="Event name")
    args = parser.parse_args()

    raw = ""

    # Priority 1: --prompt/--response shorthand (built entirely inside Python = no encoding issue)
    if args.prompt:
        data = {
            "hook_event_name": args.event,
            "prompt": args.prompt,
            "response_summary": args.response or "",
        }
        raw = json.dumps(data, ensure_ascii=False)

    # Priority 2: --json argument
    elif args.json_str:
        raw = args.json_str

    # Priority 3: stdin (legacy / other tools)
    else:
